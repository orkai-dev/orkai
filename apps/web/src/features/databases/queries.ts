...
import type { AppStatus, PaginatedResponse, PodInfo } from "@/shared/types";
import type { DatabaseBackup, DatabaseCredentials, DBVersionInfo, ManagedDB } from "./types";

export const dbKeys = {
  detail: (id: string) => ["databases", id] as const,
};

// ── All databases (global list) ───────────────────────────────────

export function useAllDatabases(page = 1, perPage = 20, search?: string, status?: string) {
  const params = new URLSearchParams({ page: String(page), per_page: String(perPage) });
  if (search) params.set("search", search);
  if (status) params.set("status", status);

  return useQuery({
    queryKey: ["databases", "all", page, perPage, search, status],
    queryFn: () => api.get<PaginatedResponse<ManagedDB>>(`${endpoints.databases.list()}?${params}`),
  });
}
...
export function useCreateDatabase(projectId: string) {
  ...
  return useMutation({
    ...
    meta: { errorMessage: "Failed to create database" },
  });
}
...
export function useRestoreBackup(dbId: string) {
  ...
  return useMutation({
    ...
    meta: { errorMessage: "Restore failed" },
  });
}
...
export function useUpdateExternalAccess(dbId: string) {
  ...
  return useMutation({
    ...
    meta: { errorMessage: "Failed to update external access" },
  });
}
