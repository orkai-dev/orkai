...
import { useAppPods, useAppStatus } from "@/features/apps";
...
import { fmtCPU, fmtMem, parseToMiB, parseToMillicores } from "@/lib/resources";
...
function ChartTooltip({
  ...
  return (
    <div className="border bg-popover px-3 py-2 text-xs">
...
function PodCard({ pod }: { pod: PodInfo }) {
  ...
  const cpuColor = cpuPct > 90 ? "bg-destructive" : cpuPct > 70 ? "bg-warning" : "bg-primary";
  const memColor = memPct > 90 ? "bg-destructive" : memPct > 70 ? "bg-warning" : "bg-primary";
    ...
    <div className="border p-3">
      <div className="mb-2.5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span
            className={`status-dot ${statusDotColor(pod.phase === "Running" ? "running" : pod.phase === "Pending" ? "pending" : pod.phase === "Failed" ? "failed" : "idle")}`}
          />
