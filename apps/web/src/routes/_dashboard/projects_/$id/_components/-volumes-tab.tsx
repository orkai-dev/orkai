...
import { useAppTargetCapabilities, useDeploy, useUpdateApp } from "@/features/apps";
import type { App, VolumeMount } from "@/features/apps/types";
import { useStorageClasses } from "@/features/cluster/queries";
import type { StorageClassInfo } from "@/features/cluster/types";
import { COMMON_MOUNT_PATHS, VOLUME_SIZE_OPTIONS } from "@/lib/constants";

function filterStorageClasses(
  classes: StorageClassInfo[] | undefined,
  allowlist: string[] | undefined,
): StorageClassInfo[] {
  if (!classes?.length) return [];
  if (!allowlist?.length) return classes;
  const allowed = new Set(allowlist);
  return classes.filter((sc) => allowed.has(sc.name));
}
...
export function VolumesTab({ app, appId }: { app: App; appId: string }) {
  ...
  const { data: targetCaps } = useAppTargetCapabilities(appId);
  const allowedStorageClasses = targetCaps?.allowed_storage_classes;
  const selectableStorageClasses = useMemo(
    () => filterStorageClasses(storageClasses, allowedStorageClasses),
    [storageClasses, allowedStorageClasses],
  );
  const defaultStorageClass = targetCaps?.default_storage_class;
  ...
  const serialize = (v: VolumeMount[]) =>
    v.map((x) => `${x.mount_path}:${x.size}:${x.storage_class ?? ""}`).join(",");
  ...
  function addVolume() {
    ...
    const vol: VolumeMount = { name, mount_path: path, size: newSize };
    if (newStorageClass) vol.storage_class = newStorageClass;
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      {vol.storage_class && <span>Class: {vol.storage_class}</span>}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {(storageClasses?.length ?? 0) > 0 && (
            <div className="space-y-1.5">
              <Label className="text-xs">
                Storage Class <span className="text-muted-foreground">(optional)</span>
              </Label>
              {(allowedStorageClasses?.length ?? 0) > 0 && selectableStorageClasses.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  No permitted storage classes are available on this cluster.
                </p>
              ) : (
                selectableStorageClasses.length > 0 && (
                  <Select
                    value={newStorageClass || "__default__"}
                    onValueChange={(v) => setNewStorageClass(v === "__default__" ? "" : v)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Cluster default" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="__default__">
                        {defaultStorageClass
                          ? `Target default (${defaultStorageClass})`
                          : "Cluster default"}
                      </SelectItem>
                      {selectableStorageClasses.map((sc) => (
                        <SelectItem key={sc.name} value={sc.name}>
                          {sc.name}
                          {sc.is_default ? " (cluster default)" : ""}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              {(allowedStorageClasses?.length ?? 0) > 0 && selectableStorageClasses.length > 0 && (
                <p className="text-xs text-muted-foreground">
                  Only storage classes permitted for this deploy target are shown.
                </p>
              )}
            </div>
          )}
