...
import type { App, Domain } from "@/features/apps/types";
import { statusDotColor, statusVariant } from "@/lib/constants";
import type { AppStatus, PodInfo } from "@/shared/types";
...
function FlowNode({
  ...
  return (
    <div className={`border bg-card p-3 transition-all ${highlight ? "border-primary/40" : ""}`}>
      <div className="mb-2 flex items-center gap-2">
        <div className="flex h-6 w-6 items-center justify-center border border-primary/30 bg-primary/10">
              ...
              <div className="space-y-1.5">
                {pods.map((pod) => (
                  <div key={pod.name} className="flex items-center gap-2">
                    <span
                      className={`status-dot ${statusDotColor(pod.phase === "Running" ? "running" : pod.phase === "Pending" ? "pending" : "failed")}`}
                    />
              ...
              <div className="space-y-1.5">
                {domains.map((d) => (
                  <div key={d.id} className="flex items-center gap-1.5">
                    {d.tls && <Lock className="h-3 w-3 text-success" />}
                    <a
                    ...
                    <span
                      className={`status-dot ${statusDotColor(d.ingress_ready ? "running" : "pending")}`}
                    />
                  </div>
                ))}
              </div>
        ...
        <FlowNode icon={Container} title={`Pods (${readyReplicas}/${desiredReplicas})`}>
          <div className="space-y-1">
            {pods.map((pod) => (
              <div key={pod.name} className="flex items-center gap-2">
                <span
                  className={`status-dot ${statusDotColor(pod.phase === "Running" ? "running" : pod.phase === "Failed" ? "failed" : "pending")}`}
                />
