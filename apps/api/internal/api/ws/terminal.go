...
func NewTerminalHandler(s store.Store, targets *orchestrator.TargetRegistry, logger *slog.Logger) *TerminalHandler {
	return &TerminalHandler{store: s, targets: targets, logger: logger}
}
...
func (h *TerminalHandler) Handle(c *gin.Context) {
	...
	if !target.Capabilities().Has(orchestrator.CapExec) {
		_ = conn.WriteMessage(websocket.TextMessage, []byte("Error: deploy target does not support exec\r\n"))
		return
	}
	ex, err := orchestrator.AsCapability[orchestrator.Execer](target, orchestrator.CapExec)
	if err != nil {
		_ = conn.WriteMessage(websocket.TextMessage, []byte("Error: "+err.Error()+"\r\n"))
		return
	}

	session, err := ex.ExecTerminal(c.Request.Context(), app, opts)
	if err != nil {
		_ = conn.WriteMessage(websocket.TextMessage, []byte("Error: "+err.Error()+"\r\n"))
		return
	}
