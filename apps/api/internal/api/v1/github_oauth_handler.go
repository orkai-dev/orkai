...
// SetupManifest godoc
// @Summary      Get GitHub App setup manifest
// @Tags         oauth
// @Produce      json
// @Param        org query string false "GitHub organization"
// @Success      200 {object} map[string]interface{}
// @Failure      401 {object} apidocs.ProblemDetail
// @Security     BearerAuth
// @Router       /auth/github/setup [get]
func (h *GitHubOAuthHandler) SetupManifest(c *gin.Context) {
	...
	// Generate unique app name: orkai-{short-host}-{hex4}
	...
	appName := fmt.Sprintf("orkai-%s-%x", hostPart, randBytes)

	// Build manifest — hook_attributes.url is REQUIRED by GitHub
	manifest := map[string]any{
		"name":         appName,
		"url":          baseURL,
		"description":  "Orkai PaaS — deploy and manage applications on K3s",
		"public":       false,
...
// @Tags         oauth
// @Param        code query string false "Setup code"
// @Param        state query string false "CSRF state"
// @Success      302
// @Router       /auth/github/setup/callback [get]
func (h *GitHubOAuthHandler) SetupCallback(c *gin.Context) {
	...
	// Verify anti-CSRF state unconditionally. A missing or unknown state must
	// abort before any manifest exchange that would overwrite github_app_* settings.
	if state == "" {
...
// Connect godoc
// @Summary      Start GitHub OAuth connect flow
// @Tags         oauth
// @Produce      json
