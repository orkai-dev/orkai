...
func NewWebhookHandler(s store.Store, deploySvc *service.DeployService, prov *providers.Registry, logger *slog.Logger) *WebhookHandler {
	return &WebhookHandler{store: s, deploySvc: deploySvc, providers: prov, logger: logger}
}
...
// @Router       /webhooks/github/{appId} [post]
func (h *WebhookHandler) GitHub(c *gin.Context) {
	...
	// Verify webhook via the git provider. When a secret is configured the
	// signature is mandatory — a missing header must not bypass verification.
	if app.WebhookSecret != "" {
		gp, gerr := h.providers.Git("github")
		if gerr != nil || !gp.VerifyWebhook(app.WebhookSecret, body, c.Request.Header) {
			h.logger.Warn("webhook signature missing or invalid", slog.String("app", app.Name))
...
// @Success      200 {object} map[string]interface{}
// @Failure      400 {object} apidocs.ProblemDetail
// @Router       /webhooks/gitlab/{appId} [post]
func (h *WebhookHandler) GitLab(c *gin.Context) {
	...
	if app.WebhookSecret != "" {
		gp, gerr := h.providers.Git("gitlab")
		if gerr != nil || !gp.VerifyWebhook(app.WebhookSecret, body, c.Request.Header) {
