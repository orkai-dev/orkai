...
func (s *NotificationService) TestChannel(ctx context.Context, orgID uuid.UUID, channelType string) error {
	...
	return s.sendToChannel(ctx, ch, "Orkai Test", "Orkai test notification", "info")
}
...
func (s *NotificationService) Notify(ctx context.Context, orgID uuid.UUID, event model.NotifyEvent, title, message string) error {
	...
	for i := range channels {
		...
		if !channelAllowsEvent(ch, event) {
			continue
		}
...
// channelAllowsEvent returns true when the channel has no event filter or the
// event is listed in config.events. Absent or unparseable config -> all events.
func channelAllowsEvent(ch *model.NotificationChannel, event model.NotifyEvent) bool {
	var cfg struct {
		Events *[]string `json:"events"`
	}
	if err := json.Unmarshal(ch.Config, &cfg); err != nil || cfg.Events == nil {
		return true
	}
...
func (s *NotificationService) SendInvitationEmail(ctx context.Context, toEmail, role, inviteURL string) error {
	...
	subject := "You've been invited to join Orkai"
	htmlBody := fmt.Sprintf(`<html><body style="font-family:sans-serif;color:#333;max-width:600px;margin:0 auto">
<h2 style="color:#2563eb">Orkai Team Invitation</h2>
<p>You've been invited to join a Orkai team as <strong>%s</strong>.</p>
...
func (s *NotificationService) TestSMTP(ctx context.Context) error {
	...
	subject := "Orkai SMTP Test"
	body := fmt.Sprintf("Subject: %s\r\nFrom: %s\r\nTo: %s\r\nMIME-Version: 1.0\r\nContent-Type: text/plain; charset=UTF-8\r\n\r\nThis is a test email from Orkai.",
...
func (s *NotificationService) sendToChannel(ctx context.Context, ch *model.NotificationChannel, title, message, severity string) error {
	...
	case model.NotifyGoogleChat:
		return s.sendGoogleChat(ctx, ch, title, message, severity)
	default:
...
func (s *NotificationService) sendEmail(ctx context.Context, ch *model.NotificationChannel, title, message, severity string) error {
	...
	subject := fmt.Sprintf("[Orkai][%s] %s", strings.ToUpper(severity), title)
	body := fmt.Sprintf("Subject: %s\r\nFrom: %s\r\nTo: %s\r\nMIME-Version: 1.0\r\nContent-Type: text/plain; charset=UTF-8\r\n\r\n%s",
...
// sendGoogleChat sends a notification via Google Chat incoming webhook.
func (s *NotificationService) sendGoogleChat(_ context.Context, ch *model.NotificationChannel, title, message, severity string) error {
	var config struct {
		WebhookURL string `json:"webhook_url"`
	}
	if err := json.Unmarshal(ch.Config, &config); err != nil {
		return fmt.Errorf("invalid google chat config: %w", err)
	}
	if config.WebhookURL == "" {
		return fmt.Errorf("google chat webhook_url is required")
	}

	text := fmt.Sprintf("*[%s] %s*\n%s", strings.ToUpper(severity), title, message)
	payload, marshalErr := json.Marshal(map[string]string{"text": text})
	if marshalErr != nil {
		return fmt.Errorf("marshal google chat payload: %w", marshalErr)
	}
