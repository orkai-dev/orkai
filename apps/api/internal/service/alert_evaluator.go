...
type AlertRule struct {
	...
	Evaluate func(ctx context.Context, k8s orchestrator.KubernetesInspector) []AlertFiring
}
	...
	return []AlertRule{
		{
			...
			Evaluate: func(ctx context.Context, k8s orchestrator.KubernetesInspector) []AlertFiring {
				nodes, err := k8s.GetNodeMetrics(ctx)
				if err != nil {
					return nil
				}
		...
		},
		{
			...
			Evaluate: func(ctx context.Context, k8s orchestrator.KubernetesInspector) []AlertFiring {
				events, err := k8s.GetClusterEvents(ctx, 50)
				if err != nil {
					return nil
				}
...
func (ae *AlertEvaluator) Evaluate(ctx context.Context) {
	k8s, err := defaultK8s(ae.targets)
	if err != nil {
		return
	}
	for _, rule := range ae.rules {
		firings := rule.Evaluate(ctx, k8s)
