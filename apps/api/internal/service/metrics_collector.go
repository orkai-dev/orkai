...
type MetricsCollector struct {
	...
	orch         *orchestrator.TargetRegistry
	logger       *slog.Logger
...
// When isLeader is non-nil, only the elected leader runs write-side work.
func (c *MetricsCollector) Start(isLeader func() bool) {
	ctx, cancel := context.WithCancel(context.Background())
	c.cancel = cancel

	c.wg.Add(3)
	go c.collectLoop(ctx, isLeader)
	go c.evaluateLoop(ctx, isLeader)
	go c.cleanupLoop(ctx, isLeader)
...
func shouldRunSingleton(isLeader func() bool) bool {
	return isLeader == nil || isLeader()
}

func (c *MetricsCollector) collectLoop(ctx context.Context, isLeader func() bool) {
	defer c.wg.Done()
...
func (c *MetricsCollector) evaluateLoop(ctx context.Context, isLeader func() bool) {
	defer c.wg.Done()
	ticker := time.NewTicker(collectInterval)
	defer ticker.Stop()
	for {
...
func (c *MetricsCollector) cleanupLoop(ctx context.Context, isLeader func() bool) {
	defer c.wg.Done()
...
func (c *MetricsCollector) collect(ctx context.Context) {
	...
	k8s, kerr := defaultK8s(c.orch)
	if kerr == nil {
		nodeMetrics, err := k8s.GetNodeMetrics(ctx)
		if err == nil {
		...
		allPods, err := k8s.GetAllPods(ctx)
		if err == nil {
		...
		clusterEvents, err := k8s.GetClusterEvents(ctx, 100)
		if err == nil {
