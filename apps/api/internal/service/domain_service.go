...
func NewDomainService(s store.Store, targets *orchestrator.TargetRegistry, logger *slog.Logger, settingSvc *SettingService, notifSvc *NotificationService) *DomainService {
	return &DomainService{store: s, targets: targets, logger: logger, settingSvc: settingSvc, notifSvc: notifSvc}
}
...
func (s *DomainService) Create(ctx context.Context, appID uuid.UUID, input CreateDomainInput) (*model.Domain, error) {
	...
	ingress, err := targetIngress(s.targets, app)
	if err != nil {
		_ = s.store.Domains().Delete(ctx, domain.ID)
		return nil, err
	}
	if err := ingress.CreateIngress(ctx, domain, app); err != nil {
	...
	if status, sErr := ingress.GetIngressStatus(ctx, domain, app); sErr == nil {
...
func (s *DomainService) GenerateTraefikDomain(ctx context.Context, appID uuid.UUID) (*model.Domain, error) {
	...
	for _, d := range existing {
		if strings.HasSuffix(d.Host, "."+baseDomain) {
			...
			_ = ingress.CreateIngress(ctx, &d, app)
			return &d, nil
		}
	}
...
func (s *DomainService) Update(ctx context.Context, id uuid.UUID, host *string, forceHTTPS *bool) (*model.Domain, error) {
	...
	if hostChanged {
		...
		oldIngressName := ingress.IngressName(app, oldHost)
		if err := ingress.DeleteIngressByName(ctx, app, oldIngressName); err != nil {
			s.logger.Warn("failed to delete old ingress by current name",
				slog.String("old_host", oldHost), slog.Any("error", err))
		}
		legacyName := ingress.LegacyIngressName(app, oldHost)
		if legacyName != oldIngressName {
			if err := ingress.DeleteIngressByName(ctx, app, legacyName); err != nil {
		...
		if err := ingress.UpdateIngress(ctx, domain, app); err != nil {
...
func (s *DomainService) ListByApp(ctx context.Context, appID uuid.UUID) ([]model.Domain, error) {
	...
	for i := range domains {
		...
		// Check ingress ready
		status, sErr := ingress.GetIngressStatus(ctx, &domains[i], app)
		...
		if domains[i].TLS && domains[i].CertSecret != "" {
			expiry, cErr := ingress.GetCertExpiry(ctx, &domains[i], app)
...
func (s *DomainService) Delete(ctx context.Context, id uuid.UUID) error {
	...
	if err := ingress.DeleteIngress(ctx, domain); err != nil {
	...
	if err := s.store.Domains().Delete(ctx, id); err != nil {
		return err
	}

	notifyProjectResourceDeleted(s.notifSvc, s.store, s.logger, app.ProjectID, model.EventDomainDeleted,
		domain.Host, fmt.Sprintf("Domain %q was deleted from application %q", domain.Host, app.Name))
	return nil
}
