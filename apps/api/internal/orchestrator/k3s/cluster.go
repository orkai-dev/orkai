...
func (o *Orchestrator) GetStorageClasses(ctx context.Context) ([]orchestrator.StorageClassInfo, error) {
	scs, err := o.client.StorageV1().StorageClasses().List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, err
	}

	var result []orchestrator.StorageClassInfo
	for _, sc := range scs.Items {
		...
		result = append(result, orchestrator.StorageClassInfo{
			Name:           sc.Name,
...
func (o *Orchestrator) GetClusterTopology(ctx context.Context) (*orchestrator.ClusterTopology, error) {
	...
	// Deployments (orkai-managed)
	deps, err := o.client.AppsV1().Deployments("").List(ctx, metav1.ListOptions{
		LabelSelector: "app.kubernetes.io/managed-by=orkai",
	})
	if err == nil {
	...
	if err == nil {
		for _, p := range podList.Items {
			...
			topo.Pods = append(topo.Pods, orchestrator.TopologyPod{
				...
				IP:         p.Status.PodIP,
				AppID:      p.Labels["orkai/app-id"],
				Deployment: dep,
			})
		}
	}

	// Services (orkai-managed)
	...
	if err == nil {
		for _, s := range svcList.Items {
			...
			topo.Services = append(topo.Services, orchestrator.TopologyService{
				...
				AppID:     s.Labels["orkai/app-id"],
			})
		}
	}

	// Ingresses (orkai-managed)
...
func (o *Orchestrator) GetNodePools(ctx context.Context) ([]string, error) {
	...
	for _, n := range nodes.Items {
		if pool, ok := n.Labels["orkai/pool"]; ok && pool != "" {
			poolSet[pool] = true
		}
	}
	var pools []string
