...
func (o *Orchestrator) waitForBuildJob(ctx context.Context, namespace, jobName string, onLog orchestrator.LogCallback) (string, error) {
	...
	if onLog != nil {
		...
		go o.streamBuildLogs(logCtx, namespace, jobName, onLog, logDone)
	}
	...
	// Stop log streaming and wait for the goroutine so OnLog cannot race with the caller.
	if logCancel != nil {
		logCancel()
	}
	if logDone != nil {
		...
		case <-time.After(5 * time.Second):
			o.logger.Warn("timed out waiting for build log stream to finish", slog.String("job", jobName))
		}
	}
...
func (o *Orchestrator) streamBuildLogs(ctx context.Context, namespace, jobName string, onLog orchestrator.LogCallback, done chan struct{}) {
	defer close(done)
...
func (o *Orchestrator) CancelBuild(ctx context.Context, app *model.Application) error {
	...
	jobs, err := o.client.BatchV1().Jobs(ns).List(ctx, metav1.ListOptions{
		LabelSelector: fmt.Sprintf("orkai/app-id=%s,orkai/build=true", app.ID.String()),
	})
	if err != nil {
		return err
	}
