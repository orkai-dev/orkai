...
func (o *Orchestrator) Deploy(ctx context.Context, app *model.Application, opts orchestrator.DeployOpts) error {
	...
	for i, vol := range opts.Volumes {
		...
		if err := o.ensurePVC(ctx, ns, pvcName, vol.Size, vol.StorageClass, labels); err != nil {
	...
	// Ensure an image pull secret exists when private registry creds are supplied.
	var imagePullSecrets []corev1.LocalObjectReference
	if len(opts.ImagePullSecret) > 0 {
		if err := o.EnsureImagePullSecret(ctx, app, opts.ImagePullSecret); err != nil {
			return fmt.Errorf("ensure image pull secret: %w", err)
		}
		imagePullSecrets = []corev1.LocalObjectReference{{Name: imagePullSecretName(app)}}
	}
			...
			Template: corev1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						"orkai/deploy-time": time.Now().Format(time.RFC3339),
					},
				},
...
func (o *Orchestrator) ensurePVC(ctx context.Context, ns, name, size, storageClass string, labels map[string]string) error {
	...
	pvc := &corev1.PersistentVolumeClaim{
		...
		Spec: corev1.PersistentVolumeClaimSpec{
			...
			StorageClassName: o.resolveStorageClass(storageClass),
		},
	}
...
func (o *Orchestrator) Rollback(ctx context.Context, app *model.Application, revision int64) error {
	...
	dep.Spec.Template.Annotations["orkai/rollback-time"] = time.Now().Format(time.RFC3339)
...
func (o *Orchestrator) ConfigureHPA(ctx context.Context, app *model.Application, cfg model.AutoscalingConfig) error {
	...
	hpa := &autoscalingv2.HorizontalPodAutoscaler{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: ns,
			Labels:    map[string]string{"app.kubernetes.io/managed-by": "orkai"},
		},
...
// imagePullSecretName is the name of the dockerconfigjson Secret created for
// an app's private registry credentials.
func imagePullSecretName(app *model.Application) string {
	return appK8sName(app) + "-registry"
}

// EnsureImagePullSecret creates or updates a kubernetes.io/dockerconfigjson
// Secret for the application. If dockerConfigJSON is empty, any existing pull
// secret is deleted.
func (o *Orchestrator) EnsureImagePullSecret(ctx context.Context, app *model.Application, dockerConfigJSON []byte) error {
	ns := appNamespace(app)
	name := imagePullSecretName(app)

	if len(dockerConfigJSON) == 0 {
		err := o.client.CoreV1().Secrets(ns).Delete(ctx, name, metav1.DeleteOptions{})
		if err != nil && !errors.IsNotFound(err) {
			return err
		}
		return nil
