...
func (o *Orchestrator) CreateIngress(ctx context.Context, domain *model.Domain, app *model.Application) error {
	...
	labels := map[string]string{
		...
		"orkai/domain-id":              domain.ID.String(),
	}
...
func (o *Orchestrator) SyncIngressPorts(ctx context.Context, app *model.Application) error {
	...
	ingresses, err := o.client.NetworkingV1().Ingresses(ns).List(ctx, metav1.ListOptions{
		LabelSelector: fmt.Sprintf("orkai/app-id=%s", app.ID.String()),
	})
...
const panelIngressName = "orkai-panel"
const panelNamespace = "orkai"
...
func (o *Orchestrator) EnsurePanelIngress(ctx context.Context, domain, httpsEmail string) error {
	...
	// Ensure K8s Service + Endpoints pointing to the host (Orkai runs in Docker, not K8s)
...
// pointing to the host's IP:3000 (Orkai runs as a Docker container, not a K8s Pod).
...
func (o *Orchestrator) DeleteIngress(ctx context.Context, domain *model.Domain) error {
	...
	nsList, err := o.client.CoreV1().Namespaces().List(ctx, metav1.ListOptions{
		LabelSelector: "managed-by=orkai",
	})
	if err != nil {
		return err
	}
	...
	for _, ns := range nsList.Items {
		ingresses, err := o.client.NetworkingV1().Ingresses(ns.Name).List(ctx, metav1.ListOptions{
			LabelSelector: fmt.Sprintf("orkai/domain-id=%s", domain.ID.String()),
		})
		if err != nil {
			continue
		}
...
func (o *Orchestrator) GetIngressStatus(ctx context.Context, domain *model.Domain, app *model.Application) (*orchestrator.IngressStatus, error) {
	...
	ready := o.isIngressReady(ctx, ns, &ing)
...
// isIngressReady reports whether an ingress is serving traffic.
// Cloud LB controllers populate status.loadBalancer; Traefik on local k3s
// (--disable=servicelb) does not, so we fall back to backend endpoint checks.
func (o *Orchestrator) isIngressReady(ctx context.Context, ns string, ing *networkingv1.Ingress) bool {
	...
	for _, rule := range ing.Spec.Rules {
		...
		for _, path := range rule.HTTP.Paths {
			...
			ep, err := o.client.CoreV1().Endpoints(ns).Get(ctx, svc.Name, metav1.GetOptions{})
			if err != nil {
				continue
			}
