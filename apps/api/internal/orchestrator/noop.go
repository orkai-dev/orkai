	...
	return []PodEvent{
		{
			Type:      "Normal",
			Reason:    "Scheduled",
			Message:   fmt.Sprintf("Successfully assigned default/%s to orkai-server", podName),
			Count:     1,
...
func (n *NoopOrchestrator) GetDatabaseCredentials(ctx context.Context, db *model.ManagedDatabase) (*DatabaseCredentials, error) {
	...
	return &DatabaseCredentials{
		...
		DatabaseName:     "app",
		ConnectionString: fmt.Sprintf("postgresql://orkai:noop-password-abc123@%s:5432/app", host),
...
func (n *NoopOrchestrator) RunDatabaseBackup(ctx context.Context, db *model.ManagedDatabase, backupID uuid.UUID, transfer *ObjectTransfer) error {
...
func (n *NoopOrchestrator) RestoreDatabaseBackup(ctx context.Context, db *model.ManagedDatabase, backupID uuid.UUID, transfer *ObjectTransfer) error {
...
func (n *NoopOrchestrator) GetAllPods(ctx context.Context) ([]PodInfo, error) {
	return []PodInfo{
		{
			Name: "nginx-abc12", Phase: "Running", Node: "orkai-server",
		...
		{
			Name: "api-server-def34", Phase: "Running", Node: "orkai-server",
		...
		{
			Name: "worker-ghi56", Phase: "Running", Node: "orkai-server",
		...
		{
			Name: "coredns-jkl78", Phase: "Running", Node: "orkai-server",
		...
		{
			Name: "postgres-mno90", Phase: "Running", Node: "orkai-server",
...
func (n *NoopOrchestrator) GetClusterEvents(ctx context.Context, limit int) ([]ClusterEvent, error) {
	return []ClusterEvent{
		{
			Type: "Normal", Reason: "Scheduled", Message: "Successfully assigned default/nginx-abc12 to orkai-server",
		...
		},
		{
			...
			Namespace: "orkai-apps", InvolvedObject: "Pod/api-server-def34", Count: 3,
		...
		},
		{
			...
			Namespace: "orkai-apps", InvolvedObject: "Pod/worker-old", Count: 12,
...
func (n *NoopOrchestrator) GetPVCs(ctx context.Context) ([]PVCInfo, error) {
	return []PVCInfo{
		{
			Name: "data-postgres-0", Namespace: "orkai-apps", Status: "Bound",
			Capacity: "10Gi", StorageClass: "local-path", VolumeName: "pvc-abc123",
		},
		{
			Name: "uploads-api", Namespace: "orkai-apps", Status: "Pending",
			Capacity: "5Gi", StorageClass: "local-path", VolumeName: "",
		},
	}, nil
}

func (n *NoopOrchestrator) GetStorageClasses(ctx context.Context) ([]StorageClassInfo, error) {
	return []StorageClassInfo{
		{Name: "local-path", Provisioner: "rancher.io/local-path", IsDefault: true, AllowExpansion: false},
		{Name: "gp3", Provisioner: "ebs.csi.aws.com", IsDefault: false, AllowExpansion: true},
	}, nil
}
...
func (n *NoopOrchestrator) GetNamespaces(ctx context.Context) ([]NamespaceInfo, error) {
	return []NamespaceInfo{
		...
		{Name: "orkai-apps", Status: "Active", PodCount: 3, SvcCount: 2},
	}, nil
}
...
func (n *NoopOrchestrator) StreamLogs(ctx context.Context, app *model.Application, opts LogOpts) (io.ReadCloser, error) {
	...
	go func() {
		...
		lines := []string{
			fmt.Sprintf("[orkai] Pod %s-0 started", app.Name),
			fmt.Sprintf("[orkai] Pulling image: %s", app.DockerImage),
			"[orkai] Image pull complete",
			fmt.Sprintf("[orkai] Container %s created", app.Name),
			fmt.Sprintf("[orkai] Container %s started", app.Name),
...
func (n *NoopOrchestrator) Build(ctx context.Context, app *model.Application, opts BuildOpts) (*BuildResult, error) {
	...
	return &BuildResult{
		Image:    fmt.Sprintf("registry.orkai-system:5000/%s:noop-build", app.Name),
		Duration: 2 * time.Second,
		Logs:     logs,
	}, nil
}

func (n *NoopOrchestrator) BuildStatic(ctx context.Context, opts StaticBuildOpts) (*StaticBuildResult, error) {
	n.logger.Info("[noop] build static", slog.String("repo", opts.GitRepo))
	if opts.OnLog != nil {
		opts.OnLog("[noop] static-site builds require a Kubernetes cluster\n")
	}
	return nil, fmt.Errorf("no Kubernetes cluster available: build-enabled pages require a cluster — disable build or configure a deploy target")
}

func (n *NoopOrchestrator) BuildWorker(ctx context.Context, opts WorkerBuildOpts) (*WorkerBuildResult, error) {
	n.logger.Info("[noop] build worker", slog.String("repo", opts.GitRepo))
	if opts.OnLog != nil {
		opts.OnLog("[noop] worker deploys require a Kubernetes cluster\n")
	}
	return nil, fmt.Errorf("no Kubernetes cluster available: worker deploys require a cluster — configure a deploy target")
}

func (n *NoopOrchestrator) DeleteWorker(ctx context.Context, opts WorkerDeleteOpts) (*WorkerBuildResult, error) {
	n.logger.Info("[noop] delete worker", slog.String("repo", opts.GitRepo))
	if opts.OnLog != nil {
		opts.OnLog("[noop] worker teardown requires a Kubernetes cluster\n")
	}
	return nil, fmt.Errorf("no Kubernetes cluster available: worker teardown requires a cluster")
}
...
func (n *NoopOrchestrator) EnsureImagePullSecret(ctx context.Context, app *model.Application, dockerConfigJSON []byte) error {
	n.logger.Info("[noop] ensure image pull secret", slog.String("app", app.Name), slog.Int("bytes", len(dockerConfigJSON)))
	return nil
}
...
func (n *NoopOrchestrator) GetCleanupStats(ctx context.Context) (*CleanupStats, error) {
	...
	return &CleanupStats{
		EvictedPods:       2,
		EvictedPodNames:   []string{"default/old-pod-abc12", "orkai-apps/crashed-xyz99"},
		FailedPods:        1,
		FailedPodNames:    []string{"orkai-apps/bad-config-def34"},
		CompletedPods:     3,
		CompletedPodNames: []string{"orkai-apps/migration-001", "orkai-apps/migration-002", "default/setup-job-pod"},
		StaleReplicaSets:  2,
		StaleRSNames:      []string{"orkai-apps/api-server-old-rs1", "orkai-apps/api-server-old-rs2"},
		CompletedJobs:     1,
		CompletedJobNames: []string{"orkai-apps/backup-daily-001"},
		UnboundPVCs:       1,
		UnboundPVCNames:   []string{"orkai-apps/orphaned-pvc-data"},
	}, nil
}
...
func newNoopTerminal(name string) *noopTerminal {
	...
	go func() {
		_, _ = fmt.Fprintf(w, "orkai-noop@%s:~$ ", name)
	}()
	return t
}
