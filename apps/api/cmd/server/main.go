...
import (
	...
	"github.com/orkai-dev/orkai/apps/api/internal/server"
...
func main() {
	...
	// Deploy target registry — K3s or noop backed by DB default target
	...
	// Job queue (PGMQ)
	jobQueue := jobs.NewQueue(store.DB())
	if err := jobQueue.EnsureSchema(ctx); err != nil {
	...
	// Services
	services := service.NewContainer(store, metricsStore, targets, jwtManager, logger, cfg.Database.URL, cfg.Auth.SetupSecret, jobQueue)

	// Leader election: only one API replica runs singleton background loops.
	schedulerCtx, schedulerCancel := context.WithCancel(context.Background())
	defer schedulerCancel()
	serverElector := leader.NewElector(store.DB(), leader.ServerLeaderKey, logger)
	serverElector.TryElect(schedulerCtx)
	go serverElector.Run(schedulerCtx)

	services.Version.Start(schedulerCtx, serverElector.IsLeader)

	// Start background metrics collector
	services.Metrics.Start(serverElector.IsLeader)
	defer services.Metrics.Stop()

	// Periodically refresh ECR pull secrets (tokens expire ~12h)
	services.RegistryAuth.StartECRRefresh(schedulerCtx, serverElector.IsLeader)
	...
	go func() {
		logger.Info("starting orkai api server", slog.String("addr", cfg.ListenAddr()))
