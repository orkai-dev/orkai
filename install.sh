#!/bin/sh
# Orkai installer — https://github.com/orkai-dev/orkai
# Usage: curl -sSL https://get.orkai.dev | sudo sh
set -e

# ── Configuration ───────────────────────────────────────────────
ORKAI_VERSION="${ORKAI_VERSION:-latest}"
...
preflight() {
    if [ "$(id -u)" -ne 0 ]; then
        fail "Please run as root: curl -sSL https://get.orkai.dev | sudo sh"
    fi
...
services:
  orkai:
    image: ghcr.io/orkai-dev/orkai:\${ORKAI_VERSION}
    container_name: orkai
    restart: unless-stopped
    network_mode: host
    environment:
      DATABASE_URL: postgres://orkai:\${DB_PASSWORD}@127.0.0.1:54321/orkai?sslmode=disable
      JWT_SECRET: \${JWT_SECRET}
      SETUP_SECRET: \${SETUP_SECRET}
  ...
  postgres:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U orkai"]
      interval: 5s
      timeout: 3s
      retries: 10
...
COMPOSEFILE
    ...
    LIB_URL="https://raw.githubusercontent.com/orkai-dev/orkai/main/deploy/upgrade-lib.sh"
    ...
    if ! docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" pull 2>&1; then
        ...
        if docker image inspect "ghcr.io/orkai-dev/orkai:${ORKAI_VERSION}" >/dev/null 2>&1; then
            warn "Pull failed but local image found — using it"
        else
            fail "Failed to pull ghcr.io/orkai-dev/orkai:${ORKAI_VERSION}. Check your internet connection."
        fi
    fi

    # Start PG first, wait for healthy, then start Orkai
    info "Starting PostgreSQL..."
    docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" up -d postgres
    for i in $(seq 1 30); do
        if docker exec orkai-postgres pg_isready -U orkai >/dev/null 2>&1; then break; fi
        sleep 2
    done
    docker exec orkai-postgres pg_isready -U orkai >/dev/null 2>&1 || fail "PostgreSQL failed to start"
    ok "PostgreSQL ready"

    info "Starting Orkai..."
    docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" up -d orkai
    ...
    fail "Orkai failed to start after 180s. Check: docker compose -f $COMPOSE_FILE logs"
}
...
summary() {
    ...
    printf "\n"
    printf "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    printf "${GREEN}  Orkai is ready!${NC}\n"
    printf "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    printf "\n"
...
main() {
    printf "\n"
    printf "${CYAN}  ⛵ Orkai Installer${NC}\n"
